...
def create_app() -> Flask:
    """Simplified application factory - no database"""
    ...
    async def analyze_ip():
        """Real-time IP analysis endpoint - no storage"""
        try:
            ...
            if cached_analysis:
                if cached_analysis.should_block:
                    app_stats['blocked_ips'] += 1
            ...
            # Cache results (but don't store in database)
    ...
    def get_stats():
        ...
        cache_stats = cache_manager.get_cache_stats()
        uptime = datetime.utcnow() - app_stats['started_at']
        
        return jsonify({
            "success": True,
            "data": {
                "total_requests": app_stats['total_requests'],
                "blocked_ips": app_stats['blocked_ips'],
                "block_rate": (app_stats['blocked_ips'] / max(app_stats['total_requests'], 1)) * 100,
                "uptime_seconds": int(uptime.total_seconds()),
                "cache_stats": cache_stats,
                "storage_type": "memory-only"
            }
        })
...
if __name__ == "__main__":
    print("🚀 Starting IP Guardian API - Memory Only Version")
    ...
    print(f"💾 Storage: Memory only (no database)")
